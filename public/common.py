...
class SetLog:
    ...
    logger.add(ERR_LOG_PATH, rotation="500 MB", encoding='utf-8', level='ERROR')
    # logger.remove()  # 删去import logger之后自动产生的handler，不删除的话会出现重复输出的现象
...
class DelReport:
    ...
    def run_del_report(self, ) -> None:
        ...
        if is_clean_report:  # 如果为 True 清除 PRPORE_ALLURE_DIR、 PRPORE_JSON_DIR 、PRPORE_SCREEN_DIR 路径下报告
